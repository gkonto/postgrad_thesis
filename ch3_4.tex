...
\textbf{\en{(CPU)}} και μονάδων επεξεργασίας γραφικών στον ίδιο δίαυλο, με κοινόχρηστη μνήμη και
...
συσκευή (\emph{\en{host device}}). Η κεντρική συσκευή είναι υπεύθυνη για την μεταφορά του κώδικα και των δεδομένων στον
...
Οπως δείχνει το παραπάνω παράδειγμα, ένα νήμα του εξυπηρετητή (κύρια συσκευή - \emph{\en{host}}) συναντάει την οδηγία
...
\emph{\en{target}}, οι μεταβλητές \emph{\en{A, B, C}} \emph{"αντιστοιχίζονται"} στον επιταχυντή. Η φράση
\emph{\en{mapped}} είναι το εργαλείο που χρησιμοποιεί το \emph{\en{OpenMP}} για να εξασφαλίσει την πρόσβαση της
...
\end{lstlisting}
\selectlanguage{greek}
Η οδηγία \emph{\en{target}} δημιουργεί μια διεργασία που εκτελείται στον επιταχυντή. Η διεργασία για τον εξυπηρετητή
